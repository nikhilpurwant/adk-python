...
OIDC_CONFIG_URL = os.environ.get('OIDC_CONFIG_URL', 'http://localhost:5000/.well-known/openid-configuration')
...
def token_required(f):
    ...
    def decorated_function(*args, **kwargs):
        ...
        if not auth_header or not auth_header.startswith('Bearer '):
            return {"error": True, "data": None, "message": "Missing or invalid Authorization header."}, 401
...
def get_hotels():
    ...
    hotels, error_message = hotel_booker.get_available_hotels(get_db().cursor(), location)

    if hotels is not None:
        return jsonify({"error": False, "data": hotels, "message": "Successfully retrieved hotels."}), 200
    else:
...
def book_room():
    ...
    if not all([hotel_id, guest_name, check_in_date, check_out_date, num_rooms]):
        return jsonify({"error": True, "data": None, "message": "Missing required booking information."}), 400

    booking_id, error_message = hotel_booker.book_a_room(conn, hotel_id, guest_name, check_in_date, check_out_date, num_rooms)

    if booking_id:
        return jsonify({"error": False, "data": {"booking_id": booking_id}, "message": "Booking successful!"}), 200
    else:
...
def get_details():
    ...
    if not booking_id and not guest_name:
        return jsonify({"error": True, "data": None, "message": "Please provide either a booking ID or a guest name."}), 400

    details, error_message = hotel_booker.get_booking_details(get_db().cursor(), booking_id=booking_id, guest_name=guest_name)

    if details:
        return jsonify({"error": False, "data": details, "message": "Booking details retrieved successfully."}), 200
    else:
