...
class HotelBooker:
    ...
    def _initialize_db(self):
        ...
        try:
            ...
            for booking_data in initial_bookings_data:
                ...
                cursor.execute("UPDATE bookings SET custom_booking_id = ? WHERE id = ?", (custom_id, booking_id_int))
            conn.commit()
    ...
    def get_available_hotels(self, cursor, location=None):
        ...
        query = "SELECT id, name, location, available_rooms, price_per_night FROM hotels WHERE available_rooms > 0"
        params = []
        if location:
    ...
    def book_a_room(self, conn, hotel_id, guest_name, check_in_date, check_out_date, num_rooms):
        """Books a room in a specified hotel."""
        cursor = conn.cursor()
        try:
            cursor.execute("SELECT available_rooms, price_per_night FROM hotels WHERE id = ?", (hotel_id,))
            hotel_info = cursor.fetchone()

            if not hotel_info:
                return None, f"Hotel with ID {hotel_id} not found."

            available_rooms, price_per_night = hotel_info['available_rooms'], hotel_info['price_per_night']
            if available_rooms < num_rooms:
                return None, f"Not enough rooms available at hotel ID {hotel_id}. Available: {available_rooms}, Requested: {num_rooms}"
            ...
            cursor.execute("UPDATE hotels SET available_rooms = ? WHERE id = ?",
                                (available_rooms - num_rooms, hotel_id))

            cursor.execute("""
                INSERT INTO bookings (hotel_id, guest_name, check_in_date, check_out_date, num_rooms, total_price)
                VALUES (?, ?, ?, ?, ?, ?)
            """, (hotel_id, guest_name, check_in_date, check_out_date, num_rooms, total_price))

            booking_id_int = cursor.lastrowid
            custom_booking_id = f"HB-{booking_id_int}"

            cursor.execute("UPDATE bookings SET custom_booking_id = ? WHERE id = ?",
    ...
    def get_booking_details(self, cursor, booking_id=None, guest_name=None):
        ...
        if booking_id:
            ...
            return None, "Please provide either a booking ID or a guest name to retrieve booking details."
        ...
        try:
            ...
            if not rows:
                return None, f"No booking found for the given criteria (ID: {booking_id}, Name: {guest_name})."
