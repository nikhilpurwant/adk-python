...
try:
    ...
    JWKS = None
    print("jwks.json not found. The server will not be able to generate JWTs.")

PRIVATE_KEY = os.getenv("PRIVATE_KEY")
GENERATE_JWT = os.getenv("GENERATE_JWT", "true").lower() == "true"

if GENERATE_JWT and not PRIVATE_KEY:
    raise ValueError("PRIVATE_KEY environment variable must be set when GENERATE_JWT is true.")

# A simple user registry for demonstration purposes
USER_REGISTRY = {
    "john.doe": {"password": "password123", "sub": "john.doe", "profile": "I am John Doe.", "email": "john.doe@example.com"},
    "jane.doe": {"password": "password123", "sub": "jane.doe", "profile": "I am Jane Doe.", "email": "jane.doe@example.com"}
}
...
CLIENT_REGISTRY = {
    "abc123": {
        ...
        "redirect_uri": ["http://localhost:8081/callback_implicit.html", "http://localhost:8081/callback_authcode.html", "http://localhost:8081/callback_pkce.html","http://localhost:8000/dev-ui/"],
...
def create_access_token(client_id, scopes, user_sub=None):
    if GENERATE_JWT:
        payload = {
            ...
            "exp": datetime.now(timezone.utc).timestamp() + timedelta(hours=1).total_seconds(),
...
def authorize():
    ...
    if request.method == "POST":
        ...
        if not user or user["password"] != password:
            return render_template("login.html", error="Invalid username or password", client_name=auth_request['client_name'])
...
def consent():
    ...
    if request.form.get("consent") == "true":
        if response_type == "token id_token" or response_type == "id_token token":
            ...
            new_uri = urlunparse((parsed.scheme, parsed.netloc, parsed.path, parsed.params, parsed.query, urlencode(fragment_params)))
            ...
            new_uri = urlunparse((parsed.scheme, parsed.netloc, parsed.path, parsed.params, urlencode(query_params), parsed.fragment))
...
def generate_token():
    ...
    if grant_type == "client_credentials":
        ...
        if auth_code_data["redirect_uri"] != redirect_uri or auth_code_data["client_id"] != client_id:
            logging.error(f"Redirect URI or client ID mismatch")
            return jsonify({"error": "invalid_grant", "error_description": "Redirect URI or client ID mismatch"}), 400
        ...
        if 'code_challenge' in auth_code_data and auth_code_data['code_challenge']:
            if not code_verifier:
                ...
                return jsonify({"error": "invalid_request", "error_description": "Code verifier is required for PKCE flow."}), 400
            
            computed_challenge = base64.urlsafe_b64encode(hashlib.sha256(code_verifier.encode('utf-8')).digest()).decode('utf-8').replace('=', '')
            if computed_challenge != auth_code_data['code_challenge']:
                logging.error(f"PKCE code challenge mismatch.")
                return jsonify({"error": "invalid_grant", "error_description": "PKCE code challenge mismatch."}), 400

        # Create tokens based on the stored user data
        user = auth_code_data['user']
        access_token = create_access_token(client_id, auth_code_data["scopes"], user["sub"])
...
def admin_update_config():
    try:
        ...
        OPENID_CONFIG['authorization_endpoint'] = data.get('authorization_endpoint', OPENID_CONFIG['authorization_endpoint'])
        OPENID_CONFIG['jwks_uri'] = data.get('jwks_uri', OPENID_CONFIG['jwks_uri'])
        OPENID_CONFIG['token_endpoint'] = data.get('token_endpoint', OPENID_CONFIG['token_endpoint'])
        return jsonify({"success": True, "message": "OpenID configuration updated."})
...
def admin_add_user():
    try:
        ...
        if not username or not password or not sub:
            return jsonify({"success": False, "message": "Username, password, and sub are required."}), 400
...
def admin_add_client():
    try:
        ...
        if not client_id or not client_name:
            return jsonify({"success": False, "message": "Client ID and Client Name are required."}), 400
